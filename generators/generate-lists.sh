...
## Never do it on a failed push, or we would prune away a commit which exists nowhere else!
if [ "${GIT_PUSH_RESULT}" != 0 ]; then
  ...
  git -C "${PROJECT_DIR}" fetch --depth=1 origin "$(git -C "${PROJECT_DIR}" rev-parse --abbrev-ref HEAD)"
  git -C "${PROJECT_DIR}" reflog expire --expire=now --all
  git -C "${PROJECT_DIR}" gc --prune=now --quiet
  fxOK "$(git -C "${PROJECT_DIR}" rev-list --count HEAD) commit(s) left, $(du -sh "${PROJECT_DIR}.git" | cut -f1) on disk"
fi
